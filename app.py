...
class SignupHandler(BaseHandler):
    ...
    def post(self):
        ...
        suser = yield inbounddb.suspended_users.find_one({'actual': from_email}) 
        if suser:
            msg = {'template_name': 'readdressfailure', 'email': from_email, 'global_merge_vars': [{'name': 'reason', 'content': "You have either been suspended from using our services, please send mail to activate@readdress.io and phone number in subject line"}]}
            count = rclient.publish('mailer',pickle.dumps(msg))
            gen_log.info('message ' + str(msg))
            gen_log.info('message published to ' + str(count))
            self.set_status(200)
...
class ActivateAccountHandler(BaseHandler):
    ...
    def post(self):
        ...
        suspended_user = yield self.is_user_suspended(from_email)
        if (not suspended_user) or (suspended_user and suspended_user['mapped']!=(phonenum[1:]+'@'+OUR_DOMAIN)) or (suspended_user and suspended_user['suspended']!='True'):
            content = "You are not allowed to activate this account \n"
            msg = {'template_name': 'activationfailure', 'email': from_email, 'global_merge_vars': [{'name': 'reason', 'content': content }]}
            count = rclient.lpush('mailer',pickle.dumps(msg))
            gen_log.info('message ' + str(msg))
